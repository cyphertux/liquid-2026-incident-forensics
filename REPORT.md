...
## Revision after Mission 28

**Date:** 2026-09-07 (documentation alignment with [`final/28_red_team_review.md`](final/28_red_team_review.md)).

This report (§1–§13) is **preserved as the early investigation record**. Several of its closures are **superseded in emphasis** by Missions 25–28. Authoritative current state: root [`README.md`](README.md).

### What changed

| Earlier REPORT claim | After Mission 28 |
|---|---|
| Priming of exact `(P1,C1)` “NON REPRODUIT”; priming score ~35 | Still true that **P1 never verifies** (Mission 27). A **different** path is confirmed: primer `(P0,C0,G,S0)` aliases post-fix preimage with `(P1,C1,G,S1)` |
| “NEW key blocks the miniature A→B attack” | Still true for **same `(P,C)`, different script**. Unframed post-fix keying **creates** a new alias between **this** primer and f24a:1 |
| Missing “priming witness for `(P1,C1)`” as the gap | Gap is now **historical**: post-fix binary + primer in acceptor mempool/cache — not finding a context where P1 verifies |
| Complete root cause ~55/100 | Mechanism (crypto+cache locus) **CONFIRMED** in lab; historical causality **UNPROVEN** — do not read as confirmed root cause |
| Terminology risk: hash collision language | Use **preimage aliasing / non-injective unframed concatenation** — **not** “SHA-256 collision” |
| Implicit “23.3.3 still vulnerable” = pre-fix incomplete key | Correct that **23.3.3 is still pre-`c26d719`**. Do **not** claim 23.3.3 “contains the fix” |

### Preserved falsifications (still valid)

- Truncated commitment form on wire: **REFUTED**
- Primers do not contain identical rangeproof bytes as attack P1: **FACT**
- Artificial OLD A→B cache hit / NEW rejects that A→B: **FACT**
- P1 fails under real L-BTC + `6a`: **FACT**

### Mission 27 → 28 correction

Mission 27 correctly **REFUTED** “honest Verify→Set of exact `(P1,C1)`”. It over-generalized to “cache refuted as incident cause.” Mission 28 shows the cache can still be primed by a **valid** primer sharing the post-fix preimage. Historical binding remains open.

### Epistemic snapshot (do not merge layers)

1. Cryptographic / cache **mechanism**: **CONFIRMED** (Mission 28)
2. Full historical `ConnectBlock` path: **UNPROVEN**
3. Incident causality: **UNPROVEN**
4. First acceptor: **UNKNOWN**
...
See also: [`final/29_documentation_update.md`](final/29_documentation_update.md).
